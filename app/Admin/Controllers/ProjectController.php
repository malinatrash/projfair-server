...
class ProjectController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        //$grid->column('supervisor.fio', __('Supervisor id'));
    ...
    protected function form()
    {
        ...
        $form->textarea('requirements', __('Требования'))->rules('required');
        $form->textarea('customer', __('Заказчик'))->rules('required');
        $form->textarea('additional_inf', __('Доп. Инф.'))->rules('required');
        ...
        $form->select('state_id')->options(State::all()->pluck('state', 'id'))->rules('required');
        //$form->number('state_id', __('State id'));
        //$form->number('supervisor_id', __('Supervisor id'));
        //$form->number('type_id', __('Type id'));

        $form->select('type_id')->options(Type::all()->pluck('type', 'id'))->rules('required');
        $form->multipleSelect('specialities', 'Специальности')->options(Speciality::all()->pluck('name', 'id'))->rules('required');
        $form->multipleSelect('skills', 'Требуемые навыки')->options(Skill::all()->pluck('name', 'id'))->rules('required');
